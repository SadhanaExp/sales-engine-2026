...
Next.js (React + TypeScript) · Tailwind + shadcn/ui · Postgres. Ready to Contract
(Quote → Approval → Contract → E-signature → Renewal) is native to this app.
Open everything from **one link: http://localhost:3000**.
...
Wait for `▲ Next.js … http://localhost:3000`, then open **http://localhost:3000**. Ctrl+C stops the server.
...
Scheduled Tasks; an Admin additionally has Ready to Contract and the handoff.
...
- **Ready to Contract** missing from the left nav → you are signed in as a Sales User. That entry, the quote handoff and `/api/contract` are Admin-only. Sign in as `sandhya@experience.com`.
